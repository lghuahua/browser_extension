    ...
    function handleClick(e) {
      ...
      if (target && target.nodeName) {
        ...
        if (targetNodeName === 'a') {
          ...
          // yep, it's a link.
          if(target.href.match(/\d$/)){
            var new_href = window.translate_url(target.href)
            if(new_href && new_href.match(/m4a$/)){
              target.href = new_href
            }
          }
          href = target.href
    ...
    dom: (function() {
      ...
      function getAll(param1, param2) {
        ...
        if (node && node.querySelectorAll) {
          ...
          if(!results) {
            results = node.querySelectorAll(selector[1]);
          }
          // results = node.querySelectorAll(selector);
